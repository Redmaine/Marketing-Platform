...
// 18 Sep 2026: weapons, hardware and troops added. Until now this guard only
// covered the STEP 3 text-bearing objects; a concept that literally said
// "rifle" went to the generator and was only caught afterwards, at review,
// by the HARDWARE ban — one of three attempts spent. The review ban stays
// exactly as it is; this stops the attempt being wasted in the first place.
const CONCEPT_BANNED_SUBJECTS =
  /\b(shops?|shopfronts?|storefronts?|store fronts?|shutters?|retail|high street|pubs?|caf[eé]s?|restaurants?|screens?|monitors?|displays?|projections?|dashboards?|maps?|charts?|diagrams?|blueprints?|documents?|papers?|paperwork|forms?|reports?|sticky notes?|post-?its?|files?|folders?|notebooks?|books?|newspapers?|cars?|vans?|lorr(?:y|ies)|trucks?|buses|trains?|aircraft|aeroplanes?|helicopters?|ships?|boats?|vessels?|vehicles?|ambulances?|banners?|flags?|posters?|signs?|signage|signposts?|plaques?|noticeboards?|hoardings?|billboards?|uniforms?|badges?|insignia|packaging|boxes|crates?|offices?|briefing rooms?|control rooms?|operations cent(?:re|er)s?|newsrooms?|number plates?|licen[cs]e plates?|labels?|lettering|inscriptions?|graffiti|tanks?|armoured|armored|artillery|missiles?|launchers?|warships?|submarines?|drones?|rifles?|guns?|firearms?|pistols?|ammunition|weapons?|weaponry|grenades?|rockets?|mortars?|bayonets?|helmets?|body armou?r|soldiers?|troops?|infantry|military hardware|military equipment|radios?|radio handsets?|comms|antennae?|antennas?|camouflage|camo)\b/i
...
export const CRHQ_CONCEPT_FALLBACK = 'Rain-soaked concrete and weathered brick at ground level in flat grey British daylight, water standing in the cracks and the wall filling the frame'

// 18 Sep 2026. Posts about tactics and capability ("ambush coordination now
// shows capability progression", 15–17 Sep) exhausted three nights running,
// and every night's attempt 1 was the wall above — the concept model had
// failed CONCEPT_MAX_ATTEMPTS times before an image was even asked for, so
// the first real attempt was spent on a frame the reviewer rightly scored
// relevance 0.00. The wall is the right fallback for a post about a place;
// for a post about what forces can do it is a guaranteed miss. Same menu as
// STEP 1b's tactical bullet, pre-chosen, so the fallback is at least on
// subject. Matched on the post text, deterministically.
export const CRHQ_TACTICAL_TOPIC =
  /\b(ambush(?:es|ed)?|tactic(?:s|al)?|capabilit(?:y|ies)|manoeuvres?|maneuvers?|patrols?|drills?|assaults?|raids?|infantry|forces? (?:can|could)|fire ?team|platoons?|battalions?|brigades?|combat|firefights?|doctrine|training exercises?)\b/i
export const CRHQ_TACTICAL_FALLBACK = 'A terrain model on a sand table, damp earth and gravel shaped into a ridge and a valley with plain wooden pegs and white cord laid across it marking positions, photographed close and low under the flat light of a canvas awning'
export const crhqFallbackConceptFor = (postBody: string): string =>
  CRHQ_TACTICAL_TOPIC.test(String(postBody ?? '')) ? CRHQ_TACTICAL_FALLBACK : CRHQ_CONCEPT_FALLBACK

const CONCEPT_MAX_ATTEMPTS = 3

// Returns the reason a concept is unusable, or null if it is fine.
export function conceptProblem(concept: string): string | null {
...
export async function summariseToVisualConcept(
  ...
  return validate ? crhqFallbackConceptFor(body) : fallback()
}
...
export async function reconceiveAfterSubjectReject(
  ...
  const feedback =
    ...
    `Do not describe a street, pavement, wall or empty room standing in for the idea, and never a weapon, vehicle or piece of military hardware. Re-read STEP 1b and anchor the frame on ONE concrete, ` +
    `unlettered physical emblem from its menu for the institution, process or capability this post is actually about (for tactics and capability: the terrain model, the planning bench, the parapet, the treeline, the tracks — never a radio, a net or any other piece of kit, which the review stage counts as military equipment), set in the real place if one is named. ` +
...
export const IMAGE_REVIEW_SYSTEM = `You are a measurement instrument for an image review pipeline. You do not make decisions and you never say whether an image passes or fails — you report only what is observably present, as precisely as you can.
...
Many posts are about a process, an institution, an analysis or a capability rather than a photographable event, and the brand deliberately never depicts weapons, vehicles, uniforms, maps, screens or faces. For such a post the image is built around ONE physical emblem of the thing concerned, and that emblem IS the subject — score it 0.7 (the right kind of thing for it) when it matches the post's subject, not 0.3: for policing, incidents and investigations — a CCTV housing, an unlettered blue lamp, a custody-suite door, a bare interview-room table with a wall recorder, a body-worn camera on plain cloth; for courts, tribunals and inquiries — court steps and columns, an empty witness box or dock rail, an empty bench; for footage and evidence — a camera lens with its red light, a memory card or tape on bare steel; for government, ministries and policy — Whitehall stone and railings, a corridor of closed doors, an empty committee table; for readiness, shortfalls and neglect — an empty parade square, a padlocked gate, an empty stores shelf, a boarded window; for tactics, ambush, manoeuvre, drills, capability and what forces can do — a terrain model or sand table with pegs and cord, a planning bench with binoculars, torch and rope, a sandbagged parapet or earth berm, a treeline at dawn from a ditch, boot prints and tyre tracks on wet earth. Do not mark these down for the absence of weapons, vehicles or troops — those are forbidden, and their absence is deliberate. Reserve 0.3 and below for a frame that is only weather, texture, a wall, a street or a room with no emblem of the post's subject in it.`
